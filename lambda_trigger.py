...
def start_state_machine(state_machine_arn, input_payload):
...
def build_execution_input(event):
    """
    Build a short, safe RunId and merged execution input.
    Produces a dict with at least: RunId, trigger, timestamps
    """
    # Prefer ExecutionInput from event if provided
    exec_input = event.get("ExecutionInput")
    # short timestamp-based RunId: 'r<epoch_seconds>' (safe for SageMaker names and S3 keys)
    default_run_id = f"r{int(time.time())}"
    if exec_input and isinstance(exec_input, dict):
        run_id = exec_input.get("RunId") or default_run_id
        merged = {"RunId": run_id, "trigger": "ci-deploy", "timestamp": int(time.time())}
        # exec_input may override other fields, but keep RunId unless exec_input provided it
        merged.update(exec_input)
    else:
...
def lambda_handler(event, _context):
    ...
    try:
        ...
        # Try to find the State Machine ARN in pipeline outputs (supports multiple key variants)
        state_machine_arn = (
            pipeline_outputs.get("StateMachineArn")
            or pipeline_outputs.get("StateMachine")
            or pipeline_outputs.get("StateMachineARN")
        )

        execution_arn = None
        if state_machine_arn:
            # Build execution input with safe RunId
            exec_input = build_execution_input(event)
            print("[lambda_handler] Execution input:", exec_input)
            try:
                execution_arn = start_state_machine(state_machine_arn, input_payload=exec_input)
